...
class GraphHandleEndpointTest extends BaseIntegrationTest {
    ...
    def '[GET] should filter measurements by start and date end'() {
        ...
        when:
        def response = executeGet("fieldName=temperature&deviceId=nodeId-1&startDate=$startDate&endDate=$endDate")
    ...
    def '[GET] should get data based on queried field'() {
        when:
        def response = executeGet("fieldName=$fieldName&deviceId=nodeId-1")
